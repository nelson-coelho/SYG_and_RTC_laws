...
# Changes:
#
# - 2025-1-23: Initial version by Nelson Coelho
#
# ==============================================================================
...
officer_bor <- read_csv('C:/Officer_Bill_Of_Rights_db.csv')

#remove not needed columns
officer_bor <- subset(officer_bor, select = c(BREAKln, officer_bill_of_rights))
...
n_occur <- data.frame(table(JH_all_crimes_regular_officer_BoR$BREAKln))

#which rows happened more than once
n_occur[n_occur$Freq > 1,]

#list dups
JH_all_crimes_regular_officer_BoR[JH_all_crimes_MICERF_officer_BoR$BREAKln %in% n_occur$Var1[n_occur$Freq > 1],]



#check for duplicates for MICERF table
n_occur <- data.frame(table(JH_all_crimes_MICERF_officer_BoR$BREAKln))

#which rows happened more than once
n_occur[n_occur$Freq > 1,]

#list dups
JH_all_crimes_MICERF_officer_BoR[JH_all_crimes_MICERF_officer_BoR$BREAKln %in% n_occur$Var1[n_occur$Freq > 1],]


#save tables
write.csv(JH_all_crimes_regular_officer_BoR, "C:/JH_all_crimes_regular_officer_BoR.csv", row.names=FALSE)
write.csv(JH_all_crimes_MICERF_officer_BoR, "C:/JH_all_crimes_MICERF_officer_BoR.csv", row.names=FALSE)

# ==============================================================================
# FENB regular table tests

#JH cit model with interaction
model_cit_BoR_reg <- JH_cit ~ shall_issue + syg_law + officer_bill_of_rights + unemp_rate + log_murder_rate + log_police_rate + log_arrest_rate + poverty_rate + log_popstate | state
...
model_pol_BoR_reg <- JH_pol ~ shall_issue + syg_law + officer_bill_of_rights + unemp_rate + log_murder_rate + log_police_rate + log_arrest_rate + poverty_rate + log_popstate | state
...
#JH cit model with interaction
model_cit_BoR <- JH_cit_imp ~ shall_issue + syg_law + officer_bill_of_rights + unemp_rate_imp + log_murder_rate_imp + log_police_rate_imp + log_arrest_rate_imp + poverty_rate_imp + log_popstate_imp  | state
model_cit_BoR.fenegbin <- fenegbin(model_cit_BoR, data=JH_all_crimes_MICERF_officer_BoR)
summary(model_cit_BoR.fenegbin)

#JH pol model with interaction
model_pol_BoR <- JH_pol_imp ~ shall_issue + syg_law + officer_bill_of_rights + unemp_rate_imp + log_murder_rate_imp + log_police_rate_imp + log_arrest_rate_imp + poverty_rate_imp + log_popstate_imp  | state
